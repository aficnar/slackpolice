...
We can therefore eliminate some of these messages by introducing a threshold: when the distance between the channel the message was posted in and the channel that the message was classified to belong to is smaller than some value $\varepsilon$, we'll ignore the prediction of the model and the bot won't advise the user. To keep things simple, we will use a fixed, relative threshold for the entire corpus, although this could be certainly improved by, for example, considering different thresholds for different channels. 

As is usually done, we'll treat the threshold as a hyperparameter, and tune it on the validation set. It is impossible to code up some verification process that can tell us how accurately the model is performing in classifying messages as generic (for a given value of the threshold), since an actual human being needs to look at each example the model classified as generic and decide whether it is indeed generic. Because of this, and because of the fact that optimizing for a high precision or recall is a matter of user experience, we opt for the following practical approach of maximizing the accuracy likelihood: when our model is correctly classifying the messages, we would like those messages to indeed be non-generic, while for the messages our model is incorrectly classifying, we would like those to be generic. <img align="right" width="300px" src="threshold.png" hspace="20" vspace="20"> As we increase the threshold, the amount of correctly classified messages predicted to be generic will increase, while the amount of the incorrectly classified messages predicted to be non-generic will decrease, as shown in the plot on the right. A natural choice for the optimal threshold is the place where the two curves intersect, which is about 0.01 in our case.
...
<ul>
<li>One example is the identity of the user posting the message. Intuitively, if this is a veteran user, we don't expect them to go off-topic on a channel, but a new user might and we should pay more attention. We could implement that by e.g. giving a different weight to the messages from different users, based on the amount of text they entered before. Or, we could simply introduce a higher threshold for the more "trusted" users.
<li>Another potentially relevant feature is the time a user posts a message. More recent messages should be more important, and this could be taken into account by e.g. weighing them with exponentials. This would also allow for the channel topics to slowly change over time.
</ul></div>



### Augment a pre-trained word2vec <a onclick="showhide('extra4')"><font size="2">[show / hide]</font></a><br>
<div style="display:none" id="extra4"><br>Word2vec representations of words used in the current version of the bot come from a pre-trained model, i.e. the linguistic context of those words is inferred from generic Common crawl texts. But the relations between the words may change for specific channels. For example, the words *random* and *forest* may seem pretty dissimilar in a general context, but if we're in a machine learning channel, their vectors should be pretty close. It would be therefore great to try to augment the pre-trained model with the examples from the current channels, so that we retain the power of the pre-trained model and its wealth of word vectors, but also teach it to recognize relations between words specific to channels.</div>



### Density functions in w2v space <a onclick="showhide('extra5')"><font size="2">[show / hide]</font></a><br>
...
<ul>
<li>This would also perhaps allow for better flagging of the generic messages, as their clusters should have similar support in all the density functions.
<li>Finally, this approach could allow for detecting when a similar parallel topic arises in a channel. In this case, we would see the original cluster slowly splitting into two close clusters, and when the bot notices this, it could advise the Slack team owner to consider splitting the channel into two.
</ul></div>



### Making the bot more user friendly <a onclick="showhide('extra6')"><font size="2">[show / hide]</font></a><br>
<div style="display:none" id="extra6">
<br>It would be great to improve the overall user experience of the bot, for example:
<ul>
<li>Maybe sometimes you don't want the bot to bother you, so it'd nice to be able to shut it down. This can be done by e.g. implementing a [slash command](https://api.slack.com/slash-commands) on Slack. Those essentially send an `HTTP` `POST` request to a URL of choice, which can be used to start and pause the bot.
<li>With the same slash command we could then modify bot's parameters. For example, if the bot is too sensitive, we could increase its threshold.
<li>It would be perhaps better that, when the bot advises users, it does so privately, i.e. so that only the affected user can see it, so it doesn't create additional noise on the channel. These kinds of hidden messages are possible, but for that purposes the bot would need to be registered as a Slack App.
<li>Having the bot as a Slack app would also allow the bot to display buttons. When the bot suggests that the message is more appropriate for a different channel, the user could click on 'okay' and the message would get copied over to that channel. If the user wants to keep the message in the channel where it was posted, he or she could click 'no', which the bot could use for online learning (for example, for updating its thresholds).
</ul></div>
...
<ul>
<li>[FastText](https://github.com/facebookresearch/fastText) is a recent method from Facebook AI, essentially an extension of the standard word2vec that represents words as bags of characters n-grams. It can be trained really quickly and allows for online learning.
<li>The current version of the bot constructs representative messages using a simple bag-of-words approach, so it would be interesting to try and see if topic extraction via [LDA](https://en.wikipedia.org/wiki/Latent_Dirichlet_allocation) would improve accuracy. 
<li>Other potentially interesting models to consider would be [skip-thoughts](https://github.com/ryankiros/skip-thoughts), [doc2vec](https://radimrehurek.com/gensim/models/doc2vec.html) and perhaps even training an [LSTM](https://en.wikipedia.org/wiki/Long_short-term_memory) neural network.
</ul></div>

### Density functions in w2v space <a onclick="showhide('extra5')"><font size="2">[show / hide]</font></a><br>
<div id="extra5"><br>Grouping messages into different classes using their representation in a higher dimensional space sounds a lot like clustering. What if we define a simple density function in the word2vec space for each channel, made up of Gaussians centered around each word's vector with a tunable variance. For some input message we could simply calculate its value in each of the density functions and predict based on which of the density functions has the highest value (and tune the variance on the validation set). 
<ul>
<li>This would also perhaps allow for better flagging of the generic messages, as their clusters should have similar support in all the density functions.
<li>Finally, this approach could allow for detecting when a similar parallel topic arises in a channel. In this case, we would see the original cluster slowly splitting into two close clusters, and when the bot notices this, it could advise the Slack team owner to consider splitting the channel into two.
</ul></div>
...
<br>It would be great to improve the overall user experience of the bot, for example:
<ul>
<li>Maybe sometimes you don't want the bot to bother you, so it'd nice to be able to shut it down. This can be done by e.g. implementing a [slash command](https://api.slack.com/slash-commands) on Slack. Those essentially send an `HTTP` `POST` request to a URL of choice, which can be used to start and pause the bot.</li>
...
<br>In this project I started with a simple bag-of-words / tf-idf approach, after which I focused on applying (and modifying) Word Mover's Distance to quantify how similar (or distant) two documents are. There are other models on the market, and it would be interesting to see performance of some of those in this context:
<ul>
<li>[FastText](https://github.com/facebookresearch/fastText) is a recent method from Facebook AI, essentially an extension of the standard word2vec that represents words as bags of characters n-grams. It can be trained really quickly and allows for online learning.</li>
<li>The current version of the bot constructs representative messages using a simple bag-of-words approach, so it would be interesting to try and see if topic extraction via [LDA](https://en.wikipedia.org/wiki/Latent_Dirichlet_allocation) would improve accuracy. </li>
<li>Other potentially interesting models to consider would be [skip-thoughts](https://github.com/ryankiros/skip-thoughts), [doc2vec](https://radimrehurek.com/gensim/models/doc2vec.html) and perhaps even training an [LSTM](https://en.wikipedia.org/wiki/Long_short-term_memory) neural network.</li>
</ul></div>
